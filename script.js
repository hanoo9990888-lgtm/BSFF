...
function Rise() {
    // ✅ Show the message text when heart starts rising
    document.getElementById("messageText").classList.add("show");
...
// ✅ Fix audio autoplay — browsers block audio without user interaction.
//    The overlay click counts as user interaction, so audio plays reliably.
document.getElementById("startOverlay").addEventListener("click", function () {
    ...
    // Start footer border animation (synchronized with JS timer below)
    document.getElementById("footer").classList.add("active");

    // Start heart animation after 12s (matches footer border animation duration)
    setTimeout(function () {
